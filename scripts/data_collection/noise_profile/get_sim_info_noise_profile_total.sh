...
DATA_HOME=$HOME_DIR/data_total_${NODES}
...
do
	...
	if [ "$NOISE" = "noiseless" ]; then
		$SCRIPT -f $APP_DIR/noiseless-coll-$EXTR --total_interval --rank=1 -o $NOISE0_DIR/$APP_NAME-total-base-${BASE}-${i} # noiseless
	else
		$SCRIPT -f $APP_DIR/noise-${NOISE}-coll-$EXTR --total_interval --rank=1 -o $NOISE0_DIR/$APP_NAME-total-base-${BASE}-${i} # 
	fi
